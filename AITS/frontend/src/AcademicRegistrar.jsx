...
import { Box, Text, Button, Select, Flex, useToast, VStack, IconButton } from "@chakra-ui/react";
...
const AcademicRegistrarDashboard = () => {
  ...
  const assignIssue = async () => {
    if (!selectedIssue || !selectedLecturer) {
      toast({
        title: "Please select both issue and lecturer.",
        status: "warning",
        duration: 5000,
        isClosable: true,
      });
    ...
    <Flex direction="column" minHeight="100vh">
      ...
      <Flex mt="80px" flex="1">
        ...
        <Box
          ...
          position="fixed"
          top="64px" // Adjust to avoid header collision
          left="0"
        ...
        {/* Main Content */}
        <Box p={4} flex="1" ml={isOpen ? "250px" : "80px"} pb={4}>
          {activeMenu === "dashboard" && <div>Dashboard Content</div>}
          {activeMenu === "issues" && (
            <div>
              <Text fontSize="2xl" mb={4}>Manage Issues</Text>
              <Box mb={4}>
              ...
              <Select placeholder="Select Lecturer" onChange={(e) => setSelectedLecturer(e.target.value)} mt={4}>
                {lecturers.map((lecturer) => (
                  <option key={lecturer.id} value={lecturer.id}>{lecturer.fullName}</option>
                ))}
              </Select>

              <Button colorScheme="green" onClick={assignIssue} mt={4}>Assign</Button>
            </div>
          )}
          ...
          {issueDetails && (
            <Box mt={4} p={4} borderWidth="1px" borderRadius="lg">
              ...
              <Text><strong>Assigned To:</strong> {issueDetails.assigned_to?.fullName || issueDetails.assigned_to || "Not Assigned"}</Text>
